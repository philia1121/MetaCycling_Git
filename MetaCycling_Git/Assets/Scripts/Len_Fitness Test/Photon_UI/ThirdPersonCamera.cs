...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        input = new ControlMap();
        input.Prototype.Enable();

        input.Prototype.Zoom.performed += x => SetFOV(x.ReadValue<float>());
    }
    ...
    private void SetupThirdPersonCamera()
    {
        ...
        cam.position = (player == null) ?
            Vector3.zero + Quaternion.identity * pivotOffset + Quaternion.identity * camOffset :
        ...
        angleH = (player == null) ? 1f : player.eulerAngles.y;
    ...
    public void SetTouchFIeld(FixedTouchField _field)
    {
        touchField = _field;
    }
    ...
    void Update()
    {
        ...
        if (touchField != null)
        {
            angleH += Mathf.Clamp(touchField.touchDist.x, -1, 1) * horizontalAimingSpeed;
            angleV += Mathf.Clamp(touchField.touchDist.y, -1, 1) * verticalAimingSpeed;
        }
    ...
    public void SetFOV(float _scrollDelta)
    {
        float _targetFOV = this.targetFOV;
